...
// Multiple conflicts analyzing with chrono=0 version, adding the best lemma(e.g. lowest LBD) in DB
int Internal::cdcl_loop_with_inprocessing () {
  ...
  while (!res) {
    if (unsat) res = 20;
      ...
      if (allConflicts.size() > 1 and level > 0) {
	analyze_all_conflicts_nochrono_for_lowest_glue();
