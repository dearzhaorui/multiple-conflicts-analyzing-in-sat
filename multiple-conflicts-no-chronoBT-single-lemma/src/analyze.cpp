...
void Internal::analyze_all_conflicts_nochrono_for_lowest_glue () {
  assert (conflict);
  assert(!unsat);
  ...
  for (int idx = 0; idx < int(allConflicts.size()); ++idx) {
    
    Clause * reason = allConflicts[idx];
    ...
    bool discard = false;
    for (;;) {
      analyze_reason_discard_higher_glue (uip, reason, open, discard);
      if(discard) {
	break;
      }
    ...
    if (not isUnit) {
      conflict = allConflicts[idx];
      best_glue_so_far = glue;
    }
    else if (uip && isUnit) {
      conflict = allConflicts[idx];
      clear_analyzed_literals ();
