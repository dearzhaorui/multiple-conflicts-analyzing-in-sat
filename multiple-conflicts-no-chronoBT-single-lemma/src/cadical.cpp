...
int App::main (int argc, char ** argv) {
  ...
  const char * outDir = 0;  // for creating other output file during solving
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp (argv[i], "-h") ||
    ...
    } else if (!strcmp (argv[i], "-outDir")) { // outDir
      if (++i == argc) APPERR ("argument to '-outDir' missing");
      else if (outDir)
        APPERR ("multiple extension file options '-outDir %s' and '-outDir %s'",
          outDir, argv[i]);
      else outDir = argv[i];
      ...
      if (++i == argc) APPERR ("argument to '-t' missing");
        ...
        APPERR ("invalid argument in '-t %s'", argv[i]);
  ...
  if(outDir){
    ...
    solver->internal->output_dir = out_dir;
  }
  /*----------------------------------------------------------------------*/
...
void App::catch_alarm () {
  ...
  //cout << "Call to solver->terminate ();" << endl;
  solver->terminate (); // Immediate asynchronous call into solver.
#else
  //cout << "Call to timesup = true; " << endl;
