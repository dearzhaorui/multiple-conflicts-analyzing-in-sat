...
// Multiple conflicts analyzing with chrono=1 version, adding all lemmas(e.g. the ones with different UIPs and lowest LBD) in DB
int Internal::cdcl_loop_with_inprocessing () {  
  ...
  while (!res) {
    if (unsat) res = 20;
      ...
      allConflictsSet.insert(conflict);    
        
      extra_propagate();
      if (allConflicts.size() == 1 or level == 0)
	analyze();
      else {
	int lowest_DL;
	
	vector<Clause*> conflicts_at_lowDL = conflictings_at_lowestDL(lowest_DL);	  
	int idx_for_minimize = 0;
	vector<Clause*> conflicts_with_multi_UIPs = analyze_several_for_different_UIPs_no_minimization(lowest_DL, conflicts_at_lowDL, idx_for_minimize);
	if (unsat) {res = 20; break;}
	
	analyze_several(conflicts_with_multi_UIPs, idx_for_minimize);
	
      }
      allConflicts.clear(); allConflictsSet.clear();
    }
