...
inline void Internal::search_assign2 (int lit, Clause * reason) {
...
void Internal::extra_propagate () {
  START (propagate);
   
  int num_conflict_lits = 0;
  
  stats.watch_score = 0;
  
  size_t first_prop = propagated;
  --propagated;
  
  while (propagated != trail.size() and int(allConflicts.size()) < max_num_conflicts and stats.watch_score < stats.max_watch_score) {

    const int lit = -trail[propagated++];
    bool conflict_in_watch = (propagated == first_prop);       	//first watch will for sure be conflicting
	...
	if(stats.watch_score >= stats.max_watch_score) break;
	else stats.watch_score++;
	
        if (b < 0) {
	  auto aux = allConflictsSet.insert(w.clause); 		// There could be repeated clauses
	  if (aux.second) {
	    allConflicts.push_back(w.clause);
	    conflict_in_watch = true;
	    if (int(allConflicts.size()) >= max_num_conflicts) break;
	  }
	}  
        else search_assign2 (w.blit, w.clause);
    ...
    if (conflict_in_watch) {
      ++num_conflict_lits;
      if (num_conflict_lits >= max_num_conflict_lits) break;
    }
  }
