...
struct Internal {
  ...
  int max_num_conflict_lits;
  int best_trail_so_far;
  int best_glue_so_far;
  int best_size_so_far;
  int best_jump_level;
  
  unordered_set<Clause*> allConflictsSet;
  vector<Clause*> allConflicts;
  ...
  Clause * new_lemma (bool red, int glue = 0);
  ...
  void search_assign2 (int lit, Clause *);
  void search_assign_driving (int lit, Clause * reason);
  void search_assume_decision (int decision);
  void assign_unit (int lit);
  bool propagate ();
  bool propagate_final ();
  void extra_propagate();
  inline int find_forced (const Clause * c, int & forced);
  ...
  Clause * new_driving_clause_cheap (const int glue);
  int find_conflict_level (int & forced);
  int determine_actual_backtrack_level (int jump);
  
  void analyze ();
  
  int find_conflict_level_quick (Clause *c);
  vector<Clause*> conflictings_at_lowestDL (int& lowest_DL);
  vector<Clause*> analyze_several_for_different_UIPs_no_minimization (int level_conflict, vector<Clause*> conflicts_at_lowDL, int& idx_for_minimize);
  void analyze_several (vector<Clause*> conflicts, const int idx_for_minimize);

  inline int get_clause_glue (Clause *);
  
  inline vector<int> sort_lits_in_clause (Clause *);
  ...
  bool marked_subsume(int lit) const { return flags(lit).subsume; }
    ...
    int decide_more_level(); // 0=decision, 20=failed
